...
int    blockprocessing(unsigned char *s)
{
    int sizeblocks;
    long i;
    long x;
...
/* 
    ...
    while (z < 63)
    {
        printBits(sizeof(block[z]), &block[z]);
        z++;
    }*/
...
void    loadwords(int i)
{
    int x = 0;
/* 
    while (x < 16)
    {
        words[x] = (uint32_t)block[i*x*4] << 24 | (uint32_t)block[i*x*4+1] << 16 
        | (uint32_t)block[i*x*4+2] << 8 | (uint32_t)block[i*x*4+3];
        x++;
    } */
...
while (x < 16)
  {
    words[x] = 0;
    words[x] += block[(i - 1) * 64 + x * 4 + 0] << 24;
    words[x] += block[(i - 1) * 64 + x * 4 + 1] << 16;
    words[x] += block[(i - 1) * 64 + x * 4 + 2] << 8;
    words[x] += block[(i - 1) * 64 + x * 4 + 3] << 0;
    x++;
  } 
...
/*
  ...
  while (x < 16)
  {
    ...
    while (z < 15)
    {
        printBits(sizeof(words[z]), &words[z]);
        z++;
    }
